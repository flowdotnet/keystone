	...
	if (sessionStore) {
      ...
      if(sessionStore === 'connect-redis') {
			...
			if (e.code === 'MODULE_NOT_FOUND') {
				...
				// connect-redis must be explicitly installed @1.5.0,
				// and express-session has to be installed, with keystone having access to it,
				// so we special-case it here
				if (sessionStore === 'connect-redis') {
					var installName = sessionStore + "@2.1.0 express-session@1.10.0";
				} else {
					var installName = sessionStore
				}
